...
class AuthApi {
  ...
  /// Performs an HTTP 'POST /v1/auth/twoFa' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [UserTwoFaParams] userTwoFaParams (required):
  Future<Response> twoFaWithHttpInfo(
    UserTwoFaParams userTwoFaParams,
  ) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/auth/twoFa';

    // ignore: prefer_final_locals
    Object? postBody = userTwoFaParams;
  ...
  ///
  /// * [UserTwoFaParams] userTwoFaParams (required):
  Future<TwoFaReturnParams?> twoFa(
    UserTwoFaParams userTwoFaParams,
  ) async {
    final response = await twoFaWithHttpInfo(
      userTwoFaParams,
    );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
