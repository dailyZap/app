...
class Author {
  ...
  @override
  String toString() =>
      'Author[id=$id, handle=$handle, firstName=$firstName, lastName=$lastName, profilePictureUrl=$profilePictureUrl]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Author[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Author[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Author> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <Author>[];
  ...
  static Map<String, List<Author>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Author.listFromJson(
          entry.value,
          growable: growable,
        );
      }
