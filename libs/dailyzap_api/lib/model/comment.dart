...
class Comment {
  ...
  @override
  String toString() =>
      'Comment[id=$id, authorId=$authorId, content=$content, timestamp=$timestamp]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Comment[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Comment[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Comment> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<Comment>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Comment.listFromJson(
          entry.value,
          growable: growable,
        );
      }
